...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (
        ...
        if (empty($proxies)) {
            ...
            echo json_encode(["error" => "No hay servidores proxy disponibles"]);
        } else {
        ...
        echo json_encode([
            "error" => "Faltan uno o más campos de datos obligatorios",
        ]);
    }
} else {
    http_response_code(405);
    echo json_encode(["error" => "Sólo se permiten solicitudes POST"]);
}
...
function get_random_user()
{
    try {
        ...
        if ($response === false) {
            throw new Exception("No se pudieron recuperar datos aleatorios de la API.");
        }
...
function get_stripe_data()
{
    ...
    try {
        ...
        if ($response === false) {
            throw new Exception("No se pudieron recuperar los datos de la banda");
        }
...
function bypass_captcha()
{
    try {
        ...
        throw new Exception("No se pudo obtener el token captcha");
    } catch (Exception $e) {
        http_response_code(500);
        return json_encode(["error" => $e->getMessage()]);
    }
...
function get_card_info($ccn)
{
    try {
        ...
        throw new Exception("No se pudo obtener la información de la tarjeta");
    } catch (Exception $e) {
        http_response_code(500);
        return json_encode(["error" => $e->getMessage()]);
    }
    ...
    try {
        ...
        if (
            ...
            if ($payment_req == false) {
                throw new Exception("No se pudo recuperar la intención de pago o la tasa es limitada");
            }
            ...
            if ($response === false) {
                throw new Exception("La pasarela de pago Stripe falló o la tarifa es limitada");
            }
